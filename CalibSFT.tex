...
Confidence-aware RL methods typically add a calibration reward
computed from sampled responses~\citep{xu2024sayself,damani2026beyond,ma2026decoupling,yang2026scaling}.
Since this reward acts on whatever confidence values the policy already
samples, we first characterize the model's confidence distribution before
RL, then examine how RL changes it during training.
...
for further training statistics).
...
This identifies how an initially concentrated confidence
distribution can impede exploration of better-calibrated outputs, which
is consistent with the persistently low distinct-value count and
...
The same small set of confidence values therefore appears to persist
across tasks of different difficulty, failing to calibrate downstream tasks.
...
\end{equation}
\end{proposition}
Proposition~\ref{prop:confidence_support} shows that the lower bound
on Brier loss increases as the probability that confidence falls
near $p_x$ decreases.
Therefore, confidence concentration can hinder calibration
...
In our previous analysis, we showed that confidence concentration
persists during RL, since rarely sampled confidence values receive
little gradient signal. This persistent concentration further
hinders downstream calibration.
To address this problem, our key idea is to supervise the confidence
prior directly before RL, so that these values are learned rather
than left for RL to explore.
...
It learns confidence from both correct and incorrect
responses, while using correct responses to supervise reasoning and
answer generation.
...
the base model and use the task verifier to obtain correctness labels
$z_{g,i}\in\{0,1\}$.
...
the base model, while $z_{g,i}$ distinguishes correct and incorrect
...
the base model. We quantify how mixing them preserves this expectation
...
Given a prompt $x_g$ and a sampling policy, let
...
valid outputs. Figure~\ref{fig:initial_confidence_prior} separates correct
and incorrect responses, normalizing each group's density to integrate
to one. Confidence values are rounded to two decimal places before Gaussian
smoothing with bandwidth 0.04 and reflection at zero and one.
Table~\ref{tab:base_confidence_prior_statistics} reports the pooled
statistics from the rounded, unsmoothed values; Pass@1 uses all rollouts.

\begin{table}[t]
    \centering
    \caption{Overall accuracy and confidence concentration before RL.
    All entries are percentages; confidence statistics pool correct and
    incorrect responses.}
    \label{tab:base_confidence_prior_statistics}
    \begin{tabular}{lrrr}
        \toprule
        Model & Pass@1 & Top-3 mass & Confidence $<0.5$ \\
        \midrule
        Qwen3-8B & 62.01 & 93.94 & 0.61 \\
        Gemma4-E2B & 54.99 & 91.06 & 1.64 \\
        OLMo3-7B & 48.72 & 65.30 & 4.90 \\
        \bottomrule
    \end{tabular}
\end{table}
...
Fix the policy and a question $x$, and draw $G$ independent
confidence samples $C_1,\ldots,C_G$ from its conditional distribution.
Each confidence sample lies outside
$\{c:|c-p_x|<\delta\}$ with probability $1-m_\delta(x)$.
Independence gives the probability $(1-m_\delta(x))^G$ that all $G$
samples lie outside. On that event, even the sample closest to $p_x$
has squared error at least $\delta^2$, so
\begin{equation}
    \mathbb E\!\left[\min_{i\leq G}(C_i-p_x)^2\mid x\right]
    \geq\delta^2(1-m_\delta(x))^G.
\end{equation}
This statement fixes the policy and the question $x$; it does not
assume that the confidence support remains unchanged during RL.
